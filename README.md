...
仓库中的 `source=static` 是开发和离线验收用的数据源，默认隐藏，不是生产数据源。该内部字段按字符串接收；旧工作流里保存的数字索引或其他未知值会自动回退为 `civitai`，避免升级后在 ComfyUI 输入校验阶段失败。
...
4. 需要绕过缓存重新查询时，点击“刷新结果”。“下一页（执行）”会继续当前查询的 cursor 分页。画廊按钮使用 ComfyUI 的局部执行目标，只执行本节点及其必要依赖，不会重新运行工作流中的其他输出节点。
